...
export class ClienteService {
  ...
  adicionarCliente(cliente: InterfacePessoa): InterfacePessoa {
    const existingCliente = this.clientes.find(c => c.id === cliente.id);
    if (existingCliente) {
      throw new Error(`Cliente com ID ${cliente.id} já existe.`);
    }
  ...
  deletarCliente(id: number): { message: string } {
    ...
    const clienteIndex = this.clientes.findIndex(g => g.id === id);
    if (clienteIndex === -1) {
